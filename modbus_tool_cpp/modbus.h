...
#define  FC_READ_ORT900_SOE          0x0C /* (0x0C)Ã·»°SOE(ORT900) */
...
public:
    ...
    */
    ...
    /* Initializes the modbus_param_t structure for RTU.
       ...
       - stop_bits: 1, 2
    */
    void modbus_init_rtu (const char *device,
                          int baud, const char *parity, int data_bit,
                          int stop_bit, int slave);
    ...
    /* Initializes the modbus_param_t structure for TCP.
       ...
       number.
    */
    void modbus_init_tcp (const char *ip_address, int port, int slave);

    /* Define the slave number.
       The special value MODBUS_BROADCAST_ADDRESS can be used. */
    void modbus_set_slave (int slave);
     ...
     */
    void modbus_set_error_handling (error_handling_t error_handling);
    ...
    void modbus_set_debug (int boolean);
    ...
    /* Listens for any query from one or many modbus masters in TCP.
       Returns: socket
     */
    int modbus_slave_listen_tcp (int nb_connection);

    /* Waits for a connection */
    int modbus_slave_accept_tcp (int *socket);
    ...
    */
    int modbus_slave_receive (int sockfd, uint8_t *query, int select_time);
    ...
    /* Manages the received query.
       ...
       accordingly.
    */
    void modbus_slave_manage (const uint8_t *query,
    ...
    int receive_msg (int msg_length_computed, uint8_t *msg, int select_time);

    int rcv_msg (uint8_t *msg, int select_time, int wait_time);

    /* Sends a query/response over a serial or a TCP communication */
    int modbus_send (uint8_t *query, int query_length);

    int serial_send (uint8_t *query, int query_length);

    void error_treat (int code, const char *string);

    int build_query_basis (int function, int start_addr, int nb, uint8_t *query);

    int check_crc16 (uint8_t *msg, const int msg_length);

    unsigned int compute_response_length (uint8_t *query , uint8_t data_type);

    int modbus_receive (uint8_t *query, uint8_t *response, uint8_t data_type, int select_time);

    void modbus_sleep (long int s, long int us);
...
private:
    ...
    int build_query_basis_rtu (int slave, int function, int start_addr, int nb, uint8_t *query);

    /* Builds a TCP query header */
    int build_query_basis_tcp (int slave, int function,
                               int start_addr, int nb,
                               uint8_t *query);

    /* Builds a RTU response header */
    int build_response_basis_rtu (sft_t *sft, uint8_t *response);

    /* Builds a TCP response header */
    int build_response_basis_tcp (sft_t *sft, uint8_t *response);

    int build_response_basis (sft_t *sft, uint8_t *response);
    ...
    uint8_t compute_query_length_header (int function);

    /* Computes the length of the data to write in the query */
    int compute_query_length_data (uint8_t *msg);
    ...
    /* Reads IO status */
    int read_io_status (int function,
    ...
    int read_registers (int function, int start_addr,
    ...
    int set_single (int function, int addr, int value,
                    int select_time);
